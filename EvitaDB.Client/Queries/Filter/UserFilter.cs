    ...
    public UserFilter(params IFilterConstraint[] children) : base(children)
    {
        if (children.Select(x => x.Type).Any(ForbiddenTypes.Contains))
        {
            throw new EvitaInvalidUsageException(
                $"Constraint(s) {string.Join(", ", children.Select(x => x.Type).Where(ForbiddenTypes.Contains).Select(y => y.Name).Select(StringUtils.Uncapitalize))} are forbidden in {Name} query container!");
        }
    }
