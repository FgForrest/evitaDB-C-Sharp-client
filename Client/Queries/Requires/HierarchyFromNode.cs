...
public class HierarchyFromNode : AbstractRequireConstraintContainer, IHierarchyRequireConstraint
{
    ...
    private HierarchyFromNode(string outputName, IRequireConstraint[] children, params IConstraint[] additionalChildren) : base(ConstraintName, new object[]{outputName}, children, additionalChildren)
    {
        foreach (IRequireConstraint requireConstraint in children)
        {
            Assert.IsTrue(requireConstraint is HierarchyNode or IHierarchyOutputRequireConstraint or Requires.EntityFetch,
    ...
    public HierarchyFromNode(string outputName, HierarchyNode fromNode) : base(ConstraintName, new object[]{outputName}, fromNode)
    {
    }
    
    public HierarchyFromNode(string outputName, HierarchyNode fromNode, params IHierarchyOutputRequireConstraint[] requirements) : base(ConstraintName, new object[]{outputName}, new IRequireConstraint[]{fromNode}.Concat(requirements).ToArray())
    {
    }
