    ...
    where T : IConstraint
{
    ...
    public override bool Applicable => Children.Length > 0 || AdditionalChildren.Length > 0;
    ...
    private IConstraint[] ValidateAndFilterAdditionalChildren(IConstraint?[] additionalChildren)
    {
        ...
        for (int i = 0; i < additionalChildrenSize; i++)
        {
            ...
            for (int j = i + 1; j < additionalChildrenSize; j++)
            {
                ...
                {
                    throw new EvitaInvalidUsageException(
                        "There are multiple additional children of same type: " + additionalChildType + " and " +
                        comparingAdditionalChildType
                    );
                }
            }
        }
